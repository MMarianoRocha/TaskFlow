...
class PomodoroDesktopApp(QWidget):
    ...
    def handle_start(self) -> None:
        ...
        try:
            ...
            self.update_session_data(session)
    ...
    def update_session_data(self, session: dict[str, Any]) -> None:
        started_at = session.get("started_at")
        duration = session.get("duration")
        if started_at and duration is not None:
            try:
                started_at_dt = datetime.fromisoformat(started_at)
                self.session_end = started_at_dt + timedelta(minutes=duration)
            except ValueError:
                self.session_end = None
        else:
            self.session_end = None
    ...
    def update_countdown(self) -> None:
        ...
        if self.logged_user:
            ...
            if current_session:
                self.update_session_data(current_session)
                self.countdown_label.setText(f"Tempo restante: {self.remaining_time_text(self.session_end)}")
            else:
                self.countdown_label.setText("Tempo restante: --:--")
                self.session_end = None

    def get_session_end(self, session: dict[str, Any]) -> datetime | None:
        started_at = session.get("started_at")
        duration = session.get("duration")
    ...
    def remaining_time_text(self, end_time: datetime | None) -> str:
        if not end_time:
            return "00:00"
    ...
    def update_overlay_text(self) -> None:
        ...
        for session in self.active_sessions:
            end_time = self.get_session_end(session)
            remaining = self.remaining_time_text(end_time)
